 ...
 */
public class PipedExcelGenerator {

    private XSSFWorkbook workbook = null;

    private Deque<GenerateFunction<ExcelSheet>> generateFunctions = new ArrayDeque<>();
    private Deque<GenerateFunction<ExcelSheet>> working = new ArrayDeque<>();

    public PipedExcelGenerator() {
        try {
     ...
     * @return 本对象
     */
    public PipedExcelGenerator generateStage(GenerateFunction<ExcelSheet> stage) {
     ...
     * @return 本对象。
     */
    public PipedExcelGenerator generateStages(Object strategies) {
        List<GenerateFunction<ExcelSheet>> functions = UnitUtils
                .extractGenerateStages(strategies, ExcelSheet.class);
        for (GenerateFunction<ExcelSheet> func: functions) {
            generateStage(func);
        }
        return this;
    }
    ...
    public void createExcel(String sheet, List<? extends Object> data, OutputStream outputStream) throws IOException {
        ...
        ExcelSheet theSheet = new ExcelSheet(workbook,sheet);
        PipedGenerationContext context = new PipedGenerationContext(data);
        Deque<GenerateFunction<ExcelSheet>> working = this.working;

        GenerateFunction<ExcelSheet> currFunction = null;
